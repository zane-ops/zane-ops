              ...
              <SelectGroup>
                {running.map((task) => (
                  <SelectItem key={task.id} value={task.container_id}>
                    <span data-label>{task.container_id.substring(0, 12)}</span>
                  </SelectItem>
                ))}
              </SelectGroup>
              <SelectGroup>
...
type LogProps = Pick<
  DeploymentLog,
  "id" | "level" | "time" | "timestamp" | "container_id"
> & {
  content: string;
  content_text: string;
};
...
export function Log({
  ...
  const isContainerIdSelected = !!searchParams.get("container_id")?.trim();
  ...
  return (
    ...
    >
      ...
      {!isContainerIdSelected && container_id && (
        <div
