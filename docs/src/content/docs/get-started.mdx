...
7. Log in to the dashboard by connecting to the domain specified in `ZANE_APP_DOMAIN`. If hosted locally ([see instructions below](#installing-locally)), you can use http://127-0-0-1.sslip.io.
</Steps>
...
- run the deploy command with http enabled, either by accepting the prompt with `Y` when running `make deploy` or using this shortcut : 
  ```shell
  yes | make deploy
  ```
