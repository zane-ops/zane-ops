...
from .fixtures import DOCKER_COMPOSE_WEB_SERVICE, DOCKER_COMPOSE_WILDCARD_WEB_SERVICE
...
class ComposeProxyViewTestCase(ComposeStackAPITestBase):
    ...
    async def test_check_wildcard_certificate_for_compose_stack(self):
        _, stack = await self.acreate_and_deploy_compose_stack(
            content=DOCKER_COMPOSE_WILDCARD_WEB_SERVICE
        )

        # the wildcard domain is `*.zaneops.io`

        self.assertIsNotNone(stack.urls)
        response = await self.async_client.get(
            reverse(
                "zane_api:proxy.check_certificates",
                query={"domain": "hello.zaneops.io"},
            ),
        )
        jprint(response.json())
        self.assertEqual(status.HTTP_200_OK, response.status_code)
