...
I'd push back on the original notes' idea of "one Redis per server, clustered together" — for now, having a single shared cache (matching the manager) is fine, because the database, Temporal, and the API already only exist on the manager anyway — if the manager goes down, cert renewal is the least of our problems. We can add real Redis replication later, as part of a dedicated "high availability" phase.

**Update (2026-09-22):** dropped our own `docker/proxy/Dockerfile` build for the multi-server stack — `zane-proxy` in `compose.prod.yaml` now runs `ghcr.io/fredkiss3/caddy-cloudflare:2.11.4-alpine` directly, which already has `caddy-storage-redis` compiled in. No xcaddy build step needed on our side for this plugin. That image is self-managed (by Fred) outside this repo, so it's expected to already contain everything this plan needs from the proxy image (redis storage, cloudflare DNS module, etc.) — don't flag its contents as a gap in future passes over this doc without checking with him first.
...
**Update (2026-09-22): implemented.** Landed as `ZaneProxyClient.get_full_caddy_config()` ([temporal/proxy.py:800](../temporal/proxy.py#L800)), exposed at `GET /api/_proxy/config/` via `CaddyConfigAPIView` ([zane_api/views/proxy.py](../zane_api/views/proxy.py#L88-L96), `InternalZaneAppPermission`) — same idea as the `ProxyDesiredConfigAPIView` sketch above, different names/path. `default-caddy-config.json` already points at this URL. Phase 2's Caddy-sync piece can be considered done; no need to re-flag it in future passes over this doc.

The old etag-based push path (`upsert_service_url`, `remove_service_url`, `cleanup_old_service_urls`, `upsert_registry_url`, `upsert_compose_stack_service_url`, and the activities that wrap them — `expose_docker_service_to_http`, `unexpose_docker_service_from_http`, `remove_old_urls`, `remove_changed_urls_in_deployment`, `upsert_registry_url_in_proxy`) is still *defined* in `temporal/proxy.py`/`main_activities.py`/`registries.py`/`stack_activities.py` and still listed in the Worker's `activities=[...]` registration array ([workflows/__init__.py:147-168](../temporal/workflows/__init__.py#L147-L168)), but confirmed dead: no workflow (`DeployGitServiceWorkflow` etc. in `services.py`) actually calls `workflow.execute_activity(...)` on any of them anymore. Deployments no longer push to the proxy — this matches the "going all in on this" plan. Deliberately left in place for reference (not scheduled for cleanup) — don't flag this as leftover work in future passes over this doc.
