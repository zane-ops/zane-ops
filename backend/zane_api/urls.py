...
urlpatterns = [
    ...
    re_path(
        rf"^projects/(?P<project_slug>{DJANGO_SLUG_REGEX})/(?P<env_slug>{DJANGO_SLUG_REGEX})/(?P<slug>{DJANGO_SLUG_REGEX})/available-volumes/?$",
        views.AvailableVolumesListAPIView.as_view(),
        name="services.volumes.available",
    ),
    re_path(
    ...
    re_path(
        r"^http-logs/?$",
        views.HttpLogsAPIView.as_view(),
        name="http_logs",
    ),
    re_path(
        r"^http-logs"
        rf"/(?P<request_uuid>{UUID_REGEX})/?$",
        views.SingleHttpLogAPIView.as_view(),
    ...
    re_path(
        rf"^projects/(?P<project_slug>{DJANGO_SLUG_REGEX})/(?P<env_slug>{DJANGO_SLUG_REGEX})/service-details"
        rf"/(?P<service_slug>{DJANGO_SLUG_REGEX})/deployments/(?P<deployment_hash>[a-zA-Z0-9-_]+)/runtime-logs/with-context/(?P<time>\d+)?$",
        views.ServiceDeploymentRuntimeLogsWithContextAPIView.as_view(),
        name="services.deployment.runtime_logs.with_context",
    ),
    re_path(
        rf"^projects/(?P<project_slug>{DJANGO_SLUG_REGEX})/(?P<env_slug>{DJANGO_SLUG_REGEX})/service-details"
    ...
    re_path(
        rf"^projects/(?P<project_slug>{DJANGO_SLUG_REGEX})/regenerate-deploy-token/?$",
        views.RegenerateProjectDeployTokenAPIView.as_view(),
        name="projects.regenerate_deploy_token",
    ),
    re_path(
        r"^trigger-preview/project/(?P<deploy_token>[a-zA-Z0-9-_]+)/?$",
        views.ProjectWebhookDeployAPIView.as_view(),
        name="projects.trigger_monorepo_preview",
    ),
    re_path(
