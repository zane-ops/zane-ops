...
class CleanupDeploymentPreserveLatestViewTests(AuthAPITestCase):
    ...
    async def test_cleanup_queue_on_git_webhook_deploy(self):
        async with self.workflowEnvironment() as env:
            with env.auto_time_skipping_disabled():
                _, service = await self.acreate_git_service()
                # ========================
                #  Deployments to cancel  #
                # ========================
                # stop it so that it has the time to receive the signal if sent
    ...
    async def test_cleanup_queue_on_docker_webhook_deploy(self):
        async with self.workflowEnvironment() as env:
            with env.auto_time_skipping_disabled():
                _, service = await self.acreate_redis_docker_service()
                # ========================
                #  Deployments to cancel  #
                # ========================
                # stop it so that it has the time to receive the signal if sent
